...
getValue givenID givenState
  = snd (lookUp givenID givenState)

getLocals :: State -> State
getLocals [] = []
getLocals givenState@(x:xs)
  | currScope == Local = x : getLocals xs
...
getGlobals [] = []
getGlobals givenState@(x:xs)
  | currScope == Global = x : getGlobals xs
...
assignArray a i v
  = A (assignArray' aArray targetIndex changeTo)
    where
      (A aArray) = a
      (I targetIndex) = i
      (I changeTo) = v
...
updateVar toUpdate@(givenId,newVal) [] = [newBinding]
  where
    newBinding = (givenId,(Local,newVal))
updateVar toUpdate@(givenId,newVal) givenState@(x:xs)
  | givenId == currId = newBinding : xs
  | otherwise = x : updateVar toUpdate xs
    where
      (currId,(currScope,_)) = x
      newBinding = (currId,(currScope,newVal))
...
applyOp Add (I left) (I right)
  = I (left + right)
applyOp Mul (I left) (I right)
  = I (left * right)
applyOp Less (I left) (I right)
  = if left < right then (I 1) else (I 0)
applyOp Equal (I left) (I right)
  = if left == right then (I 1) else (I 0)
applyOp Index (A array) (I index)
  | searchResult == Nothing = (I 0)
  | otherwise = (I (fromJust searchResult))
    where
      searchResult = lookup index array

bindArgs :: [Id] -> [Value] -> State
-- Pre: the lists have the same length
bindArgs givenIdArray givenVals
  = zip givenIdArray (zip scopeArray givenVals)
    where
      scopeArray = replicate (length givenIdArray) Local

evalArgs :: [Exp] -> [FunDef] -> State -> [Value]
evalArgs [] _ _ = []
evalArgs exprs@(e:es) givenFunDef givenState
  = eval e givenFunDef givenState : evalArgs es givenFunDef givenState
...
eval (Const c) _ _ = c
eval (Var index) _ givenState = getValue index givenState
eval (Cond condExpr thenExpr elseExpr) givenFunDef givenState
  | evalCondExpr == (I 1) = evalThenExpr
  | otherwise = evalElseExpr
    where
      evalCondExpr = eval condExpr givenFunDef givenState
      evalThenExpr = eval thenExpr givenFunDef givenState
      evalElseExpr = eval elseExpr givenFunDef givenState
eval (OpApp operator leftExpr rightExpr) givenFunDef givenState
  = applyOp operator evalLeftExpr evalRightExpr
  where
    evalLeftExpr = eval leftExpr givenFunDef givenState
    evalRightExpr = eval rightExpr givenFunDef givenState
eval (FunApp funToExecute exprs) givenFunDef givenState
  = eval e givenFunDef (newState ++ givenState)
    where
      newState = bindArgs as vs
      vs = evalArgs exprs givenFunDef givenState
      (as,e) = lookUp funToExecute givenFunDef
...
executeStatement (Assign givenId givenVal) givenFunDef _ givenState
  = updateVar (givenId,givenVal') givenState
    where
      givenVal' = eval givenVal givenFunDef givenState
executeStatement (AssignA a i v) givenFunDef _ givenState = updateVar (a,newArray) givenState
  where
    initialArray = getValue a givenState
    newArray
      = assignArray initialArray (eval i givenFunDef givenState) (eval v givenFunDef givenState)
executeStatement (If judgeExpr ifTrueBlock ifFalseBlock) givenFunDef givenProcDef givenState
  | evalJudgeExpr == (I 1) = executeBlock ifTrueBlock givenFunDef givenProcDef givenState
  | otherwise = executeBlock ifFalseBlock givenFunDef givenProcDef givenState
    where
      evalJudgeExpr = eval judgeExpr givenFunDef givenState
executeStatement whileLoop@(While judgeExpr toExecuteBlock) givenFunDef givenProcDef givenState
  | evalJudgeExpr == (I 1)
    = executeStatement whileLoop givenFunDef givenProcDef newState
  | otherwise = givenState
    where
      evalJudgeExpr = eval judgeExpr givenFunDef givenState
...
executeBlock [] _ _ finalState = finalState
executeBlock givenBlock@(s:ss) givenFunDef givenProcDef givenState
  = executeBlock ss givenFunDef givenProcDef newState
    where
      newState = executeStatement s givenFunDef givenProcDef givenState
